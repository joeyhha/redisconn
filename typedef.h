...
#include <assert.h>
	...
	{
		struct termios tm;	// 终端属性结构体
		struct termios old;	// 终端属性结构体

		int ch;
		int fd = STDIN_FILENO;	// 标准输入文件描述符

		if (tcgetattr(fd, &tm) < 0) return 0;	// 获取终端属性，并保存到tm结构体中

		old = tm;	// 保存原始终端属性
		cfmakeraw(&tm);	// 设置终端属性为原始模式，禁用行缓冲、回显和特殊字符处理等

		if (tcsetattr(fd, TCSANOW, &tm) < 0) return 0;	// 设置终端属性

		ch = fgetc(stdin);	// 从标准输入流中获取一个字符

		if (tcsetattr(fd, TCSANOW, &old) < 0) return 0;	// 恢复原始终端属性
	...
	{
		printf("\e[%dm", (int)(color));	// 打印控制终端文本颜色的转义序列
	}
